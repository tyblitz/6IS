<template>
  <MainLayout title="Communication Purposes">
    <div class="admin-reference-container">
      
      <!-- Header Action Bar -->
      <div class="header-action-bar">
        <div>
          <h2>Communication Purposes Management</h2>
          <p class="subtitle">Maintain document purposes (e.g. Access Pass, PAS Request, R&M ICT Fund Request).</p>
        </div>
      ...
      <div class="filter-controls-card">
        <div class="filter-group search-group">
          <label for="purpSearch">Search</label>
          <div class="search-input-wrapper">
            ...
            <input
              ...
              v-model="searchQuery"
              @input="currentPage = 1"
              placeholder="Search purpose name..."
              class="input-search"
            />
          </div>
        </div>
        ...
        <div class="filter-group">
          <label for="purpStatusFilter">Status</label>
          <select id="purpStatusFilter" v-model="filterStatus" @change="currentPage = 1" class="input-select">
      ...
      <div class="table-card">
        ...
        <div v-else-if="sortedList.length === 0" class="empty-state">
          <p v-if="searchQuery || filterStatus !== 'all'">No purposes match your filters.</p>
          <p v-else>No communication purposes registered yet.</p>
        </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="item in paginatedList" :key="item.id">
                ...
                <td class="font-bold text-primary">{{ item.purpose_name }}</td>
                ...
                <td class="text-center">
                  <div class="action-buttons">
                    <button class="icon-btn edit-btn" title="Edit Purpose" @click="openEditModal(item)">
                      <ion-icon :icon="createOutline" />
                    </button>
                    <button class="icon-btn delete-btn" title="Delete Purpose" @click="handleDelete(item)">
      ...
      <div v-if="showModal" class="modal-backdrop">
        <div class="modal-card">
          <div class="modal-header">
            <h3>{{ isEdit ? 'Edit Purpose' : 'Add New Purpose' }}</h3>
            <button type="button" class="close-btn" @click="showModal = false">&times;</button>
          </div>

          <form @submit.prevent="handleSave" class="modal-body">
            <div class="form-group">
              <label for="purpName">Purpose Name <span class="required-star">*</span></label>
              <input id="purpName" v-model="form.purpose_name" type="text" placeholder="e.g. Access Pass, PAS Request" required class="input-text" />
            </div>

            <div class="form-group checkbox-group">
              <label class="checkbox-label">
                <input type="checkbox" v-model="form.is_active" />
                <span>Is Active Purpose</span>
              </label>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="save-btn" :disabled="saving">
                {{ saving ? 'Saving...' : (isEdit ? 'Update Purpose' : 'Save Purpose') }}
              </button>
            </div>
          </form>
        </div>
...
const sortColumn = ref<'id' | 'purpose_name' | 'is_active' | null>('purpose_name')
...
const filteredList = computed(() => {
  return items.value.filter(item => {
    ...
    const matchesSearch = !q || item.purpose_name.toLowerCase().includes(q)
...
function toggleSort(col: 'id' | 'purpose_name' | 'is_active') {
...
async function loadData() {
  loading.value = true
  try {
    const res = await fetch(resolveHostUrl('?view=purposes'), { credentials: 'include' })
...
function openAddModal() {
  isEdit.value = false
  form.value = { id: 0, purpose_name: '', is_active: true }
  modalError.value = ''
...
async function handleSave() {
  if (!form.value.purpose_name.trim()) {
    modalError.value = 'Purpose name is required.'
    return
  }
  ...
  try {
    const res = await fetch(resolveHostUrl('?action=save_purpose'), {
      method: 'POST',
...
async function handleDelete(item: any) {
  if (!confirm(`Are you sure you want to delete purpose '${item.purpose_name}'?`)) return

  try {
    const res = await fetch(resolveHostUrl('?action=delete_purpose'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    ...
    else alert(data.message || 'Failed to delete purpose.')
  } catch (err) {
    alert('Network error.')
  }
}
