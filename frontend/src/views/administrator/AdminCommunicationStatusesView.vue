<template>
  <MainLayout title="Communication Statuses">
    <div class="admin-reference-container">
      
      <!-- Header Action Bar -->
      <div class="header-action-bar">
        <div>
          <h2>Communication Status Management</h2>
          <p class="subtitle">Maintain document condition statuses (e.g. Pending, In Progress, Completed, Released).</p>
        </div>
      ...
      <div class="filter-controls-card">
        <div class="filter-group search-group">
          <label for="statSearch">Search</label>
        ...
        <div class="filter-group">
          <label for="statStatusFilter">Status</label>
          <select id="statStatusFilter" v-model="filterStatus" @change="currentPage = 1" class="input-select">
      ...
      <div class="table-card">
        ...
        <div v-else-if="sortedList.length === 0" class="empty-state">
          <p v-if="searchQuery || filterStatus !== 'all'">No statuses match your filters.</p>
          <p v-else>No communication statuses registered yet.</p>
        </div>
          ...
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th class="sortable-th" @click="toggleSort('status_name')">
                  <div class="th-content">
                    <span>Status Name</span>
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'status_name' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'status_name' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>
            ...
            <tbody>
              <tr v-for="item in paginatedList" :key="item.id">
                ...
                <td class="font-bold text-primary">{{ item.status_name }}</td>
      ...
      <div v-if="showModal" class="modal-backdrop">
        <div class="modal-card">
          <div class="modal-header">
            <h3>{{ isEdit ? 'Edit Communication Status' : 'Add New Status' }}</h3>
            <button type="button" class="close-btn" @click="showModal = false">&times;</button>
          </div>

          <form @submit.prevent="handleSave" class="modal-body">
            <div class="form-group">
              <label for="statName">Status Name <span class="required-star">*</span></label>
              <input id="statName" v-model="form.status_name" type="text" placeholder="e.g. Pending, In Progress, Released" required class="input-text" />
            </div>

            <div class="form-group checkbox-group">
              <label class="checkbox-label">
                <input type="checkbox" v-model="form.is_active" />
                <span>Is Active Status</span>
              </label>
            </div>
...
const sortColumn = ref<'id' | 'status_name' | 'is_active' | null>('status_name')
...
const filteredList = computed(() => {
  return items.value.filter(item => {
    ...
    const matchesSearch = !q || item.status_name.toLowerCase().includes(q)
...
function toggleSort(col: 'id' | 'status_name' | 'is_active') {
...
async function loadData() {
  loading.value = true
  try {
    const res = await fetch(resolveHostUrl('?view=statuses'), { credentials: 'include' })
...
function openAddModal() {
  isEdit.value = false
  form.value = { id: 0, status_name: '', is_active: true }
  modalError.value = ''
...
async function handleSave() {
  if (!form.value.status_name.trim()) {
  ...
  try {
    const res = await fetch(resolveHostUrl('?action=save_status'), {
      method: 'POST',
    ...
    if (data.success) {
      ...
      modalError.value = data.message || 'Failed to save status.'
    }
  } catch (err) {
    saving.value = false
...
async function handleDelete(item: any) {
  if (!confirm(`Are you sure you want to delete status '${item.status_name}'?`)) return

  try {
    const res = await fetch(resolveHostUrl('?action=delete_status'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    ...
    else alert(data.message || 'Failed to delete status.')
  } catch (err) {
    alert('Network error.')
  }
}
