<template>
  <MainLayout title="Equipment Attributes">
    <div class="admin-reference-container">
      
      <!-- Header Action Bar -->
      <div class="header-action-bar">
        <div>
          <h2>Equipment Attributes Management</h2>
          <p class="subtitle">Maintain dynamic equipment specification attribute definitions (e.g. Processor, RAM, Storage, OS, Lumens).</p>
        </div>
      ...
      <div class="filter-controls-card">
        <div class="filter-group search-group">
          <label for="attrSearch">Search</label>
          <div class="search-input-wrapper">
            ...
            <input
              ...
              v-model="searchQuery"
              @input="currentPage = 1"
              placeholder="Search attribute name, code, subtype, or data type..."
              class="input-search"
            />
          </div>
        </div>
        ...
        <div class="filter-group">
          <label for="attrSubtypeFilter">Subtype</label>
          <select id="attrSubtypeFilter" v-model="filterSubtype" @change="currentPage = 1" class="input-select">
            <option value="0">-Select-</option>
            <option v-for="st in subtypes" :key="st.id" :value="st.id">{{ st.name }}</option>
          </select>
        </div>

        <div class="filter-group">
          <label for="attrDataTypeFilter">Data Type</label>
          <select id="attrDataTypeFilter" v-model="filterDataType" @change="currentPage = 1" class="input-select">
      ...
      <div class="table-card">
        ...
        <div v-if="loading" class="loading-state">
          ...
          <p>Loading equipment attribute definitions...</p>
        </div>

        <!-- Empty State -->
        <div v-else-if="sortedList.length === 0" class="empty-state">
          <p v-if="searchQuery || filterSubtype > 0 || filterDataType !== 'all'">No attribute definitions match your filters.</p>
          <p v-else>No attribute definitions registered yet.</p>
        </div>
          ...
          <table class="data-table">
              ...
              <tr>
                ...
                <th class="sortable-th" @click="toggleSort('equipment_subtype_name')">
                  <div class="th-content">
                    ...
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'equipment_subtype_name' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'equipment_subtype_name' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>

                <th class="sortable-th" @click="toggleSort('attribute_name')">
                  <div class="th-content">
                    <span>Attribute Name</span>
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'attribute_name' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'attribute_name' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>

                <th class="sortable-th" @click="toggleSort('attribute_code')">
                  <div class="th-content">
                    <span>Attribute Key</span>
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'attribute_code' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'attribute_code' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>

                <th class="sortable-th" @click="toggleSort('data_type')">
                  <div class="th-content">
                    <span>Data Type</span>
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'data_type' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'data_type' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>

                <th class="sortable-th text-center" @click="toggleSort('is_required')">
                  <div class="th-content center-th">
                    <span>Required?</span>
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'is_required' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'is_required' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>
            ...
            <tbody>
              <tr v-for="item in paginatedList" :key="item.id">
                ...
                <td class="font-bold text-primary">{{ item.equipment_subtype_name || 'Unassigned' }}</td>
                <td class="font-semibold">{{ item.attribute_name }}</td>
                <td class="code-font">{{ item.attribute_code || '-' }}</td>
                <td>
                  <span class="type-tag">{{ item.data_type }}</span>
                </td>
                <td class="text-center">
                  <span :class="['status-badge', item.is_required ? 'status-req' : 'status-opt']">
                    {{ item.is_required ? 'Yes' : 'No' }}
                  </span>
                </td>
                <td class="text-center">
                  <div class="action-buttons">
                    <button class="icon-btn edit-btn" title="Edit Attribute Definition" @click="openEditModal(item)">
                      <ion-icon :icon="createOutline" />
                    </button>
                    <button class="icon-btn delete-btn" title="Delete Attribute Definition" @click="handleDelete(item)">
      ...
      <div v-if="showModal" class="modal-backdrop">
        <div class="modal-card">
          <div class="modal-header">
            <h3>{{ isEdit ? 'Edit Attribute Definition' : 'Add New Attribute Definition' }}</h3>
            <button type="button" class="close-btn" @click="showModal = false">&times;</button>
          </div>

          <form @submit.prevent="handleSave" class="modal-body">
            <div class="form-group">
              <label for="subSelect">Target Subtype <span class="required-star">*</span></label>
              <select id="subSelect" v-model="form.equipment_subtype_id" class="input-select" required>
            ...
            <div class="form-group">
              <label for="attrName">Attribute Name <span class="required-star">*</span></label>
              <input id="attrName" v-model="form.attribute_name" type="text" placeholder="e.g. Processor, RAM, Storage, Lumens" required class="input-text" />
            </div>

            <div class="form-group">
              <label for="attrCode">Attribute Key / Code</label>
              <input id="attrCode" v-model="form.attribute_code" type="text" placeholder="e.g. processor, ram, storage" class="input-text code-font" />
            </div>

            <div class="form-group">
              <label for="dataType">Data Type <span class="required-star">*</span></label>
              <select id="dataType" v-model="form.data_type" class="input-select" required>
            ...
            <div class="form-group checkbox-group">
              <label class="checkbox-label">
                ...
                <span>Is Required Field for Subtype</span>
              </label>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="save-btn" :disabled="saving">
                {{ saving ? 'Saving...' : (isEdit ? 'Update Attribute' : 'Save Attribute') }}
              </button>
            </div>
          </form>
        </div>
...
const filterSubtype = ref(0)
const filterDataType = ref('all')
const sortColumn = ref<'id' | 'equipment_subtype_name' | 'attribute_name' | 'attribute_code' | 'data_type' | 'is_required' | null>('equipment_subtype_name')
...
const filteredList = computed(() => {
  return items.value.filter(item => {
    ...
    const matchesSearch = !q ||
      item.attribute_name.toLowerCase().includes(q) ||
      (item.attribute_code || '').toLowerCase().includes(q) ||
      (item.equipment_subtype_name || '').toLowerCase().includes(q) ||
      (item.data_type || '').toLowerCase().includes(q)
    
    // Subtype Filter
    let matchesSubtype = true
    if (filterSubtype.value > 0) {
      matchesSubtype = Number(item.equipment_subtype_id) === Number(filterSubtype.value)
    }

    // Data Type Filter
    let matchesDataType = true
    if (filterDataType.value !== 'all') {
      matchesDataType = item.data_type === filterDataType.value
    }

    return matchesSearch && matchesSubtype && matchesDataType
  })
})
...
function toggleSort(col: 'id' | 'equipment_subtype_name' | 'attribute_name' | 'attribute_code' | 'data_type' | 'is_required') {
...
async function loadData() {
  ...
  const [stRes, defRes] = await Promise.all([
    fetchEquipmentSubtypes(),
    fetch(resolveHostUrl('inventory/index.php?view=attribute_definitions'), { credentials: 'include' }).then(r => r.json())
  ])

  if (stRes.success && stRes.data) {
    subtypes.value = stRes.data
  }

  if (defRes.success && defRes.data) {
    items.value = defRes.data
  }
...
function openAddModal() {
  ...
  form.value = {
    id: 0,
    equipment_subtype_id: subtypes.value[0]?.id || 1,
    attribute_name: '',
    attribute_code: '',
    data_type: 'text',
    is_required: false,
...
function openEditModal(item: AttributeDefinition) {
  isEdit.value = true
  form.value = {
    id: item.id,
    equipment_subtype_id: item.equipment_subtype_id,
...
async function handleSave() {
  if (!form.value.equipment_subtype_id || form.value.equipment_subtype_id <= 0) {
    modalError.value = 'Please select a target equipment subtype.'
    return
  }
  if (!form.value.attribute_name.trim()) {
    modalError.value = 'Attribute name is required.'
    return
  }
  ...
  const res = await saveAttributeDefinition({
    ...
    attribute_code: form.value.attribute_code || form.value.attribute_name.toLowerCase().replace(/[^a-z0-9]/g, '_'),
  ...
  if (res.success) {
    ...
    modalError.value = res.message || 'Failed to save attribute definition.'
  }
}

async function handleDelete(item: AttributeDefinition) {
  if (!confirm(`Are you sure you want to delete attribute definition '${item.attribute_name}'?`)) return

  const res = await deleteAttributeDefinition(item.id)
  if (res.success) {
    loadData()
  } else {
    alert(res.message || 'Failed to delete attribute definition.')
  }
}
...
.action-main-btn {
  background: var(--color-primary-light); color: #ffffff; border: none; padding: 10px 18px; border-radius: var(--radius-sm);
...
.type-tag { background: #f1f5f9; color: #475569; padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight: 700; text-transform: capitalize; }
.status-badge { display: inline-block; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 700; }
.status-req { background: #fff7ed; color: #c2410c; }
.status-opt { background: #f0fdf4; color: #16a34a; }
