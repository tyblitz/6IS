<template>
  <MainLayout title="Equipment Subtypes">
    <div class="admin-reference-container">
      
      <!-- Header Action Bar -->
      <div class="header-action-bar">
        <div>
          <h2>Equipment Subtypes Management</h2>
          <p class="subtitle">Maintain equipment subtypes (e.g. Desktop, Printer, AVR, Projector, Mixer, Speaker) and their parent category assignments.</p>
        </div>
      ...
      <div class="filter-controls-card">
        <div class="filter-group search-group">
          <label for="subTypeSearch">Search</label>
          <div class="search-input-wrapper">
            ...
            <input
              ...
              v-model="searchQuery"
              @input="currentPage = 1"
              placeholder="Search subtype, code, or parent category..."
              class="input-search"
            />
          </div>
        </div>
        ...
        <div class="filter-group">
          <label for="subTypeCatFilter">Equipment Category</label>
          <select id="subTypeCatFilter" v-model="filterType" @change="currentPage = 1" class="input-select">
        ...
        <div class="filter-group">
          <label for="subTypeStatus">Status</label>
          <select id="subTypeStatus" v-model="filterStatus" @change="currentPage = 1" class="input-select">
      ...
      <div class="table-card">
        ...
        <div v-if="loading" class="loading-state">
          ...
          <p>Loading equipment subtypes...</p>
        </div>

        <!-- Empty State -->
        <div v-else-if="sortedList.length === 0" class="empty-state">
          <p v-if="searchQuery || filterType > 0 || filterStatus !== 'all'">No equipment subtypes match your filters.</p>
          <p v-else>No equipment subtypes registered yet.</p>
        </div>
          ...
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th class="sortable-th" @click="toggleSort('equipment_type_name')">
                  <div class="th-content">
                    <span>Parent Category</span>
                    <span class="sort-icon">
                      <ion-icon v-if="sortColumn === 'equipment_type_name' && sortDirection === 'asc'" :icon="arrowUpOutline" />
                      <ion-icon v-else-if="sortColumn === 'equipment_type_name' && sortDirection === 'desc'" :icon="arrowDownOutline" />
                      <ion-icon v-else :icon="swapVerticalOutline" class="inactive-sort" />
                    </span>
                  </div>
                </th>
            ...
            <tbody>
              <tr v-for="item in paginatedList" :key="item.id">
                ...
                <td>
                  <span class="category-tag">{{ item.equipment_type_name || 'Unassigned' }}</span>
                </td>
                ...
                <td class="text-center">
                  <div class="action-buttons">
                    <button class="icon-btn edit-btn" title="Edit Subtype" @click="openEditModal(item)">
                      <ion-icon :icon="createOutline" />
                    </button>
                    <button class="icon-btn delete-btn" title="Delete Subtype" @click="handleDelete(item)">
      ...
      <div v-if="showModal" class="modal-backdrop">
        <div class="modal-card">
          <div class="modal-header">
            <h3>{{ isEdit ? 'Edit Equipment Subtype' : 'Add New Subtype' }}</h3>
            <button type="button" class="close-btn" @click="showModal = false">&times;</button>
          </div>

          <form @submit.prevent="handleSave" class="modal-body">
            <div class="form-group">
              <label for="parentType">Parent Category <span class="required-star">*</span></label>
              <select id="parentType" v-model="form.equipment_type_id" class="input-select" required>
                <option value="0" disabled>Select Parent Equipment Category...</option>
                <option v-for="t in equipmentTypes" :key="t.id" :value="t.id">{{ t.name }}</option>
              </select>
            </div>

            <div class="form-group">
              <label for="subName">Subtype Name <span class="required-star">*</span></label>
              <input id="subName" v-model="form.name" type="text" placeholder="e.g. Desktop, Printer, Mixer" required class="input-text" />
            </div>

            <div class="form-group">
              <label for="subCode">Subtype Code</label>
              <input id="subCode" v-model="form.code" type="text" placeholder="e.g. DESK, PRN" class="input-text code-font" />
            </div>

            <div class="form-group checkbox-group">
              <label class="checkbox-label">
                <input type="checkbox" v-model="form.is_active" />
                <span>Is Active Subtype</span>
              </label>
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="save-btn" :disabled="saving">
                {{ saving ? 'Saving...' : (isEdit ? 'Update Subtype' : 'Save Subtype') }}
              </button>
            </div>
          </form>
        </div>
...
import { fetchEquipmentTypes, fetchEquipmentSubtypes, saveEquipmentSubtype, deleteEquipmentSubtype } from '../../services/inventoryService'
import type { EquipmentType, EquipmentSubtype } from '../../types/inventory'

const items = ref<EquipmentSubtype[]>([])
...
const sortColumn = ref<'id' | 'equipment_type_name' | 'name' | 'code' | 'is_active' | null>('name')
...
const filteredList = computed(() => {
  return items.value.filter(item => {
    ...
    if (filterType.value > 0) {
      matchesType = Number(item.equipment_type_id) === Number(filterType.value)
    }
...
function toggleSort(col: 'id' | 'equipment_type_name' | 'name' | 'code' | 'is_active') {
...
function openAddModal() {
  ...
  form.value = {
    id: 0,
    equipment_type_id: equipmentTypes.value[0]?.id || 1,
    name: '',
    code: '',
    is_active: true
  }
...
function openEditModal(item: EquipmentSubtype) {
  isEdit.value = true
  form.value = {
    id: item.id,
...
async function handleSave() {
  if (!form.value.equipment_type_id || form.value.equipment_type_id <= 0) {
    modalError.value = 'Please select a parent equipment category.'
    return
  }
  if (!form.value.name.trim()) {
    modalError.value = 'Subtype name is required.'
    return
  }
  ...
  const res = await saveEquipmentSubtype({
  ...
  if (res.success) {
    ...
    modalError.value = res.message || 'Failed to save equipment subtype.'
  }
}

async function handleDelete(item: EquipmentSubtype) {
  if (!confirm(`Are you sure you want to delete subtype '${item.name}'?`)) return

  const res = await deleteEquipmentSubtype(item.id)
  if (res.success) {
    loadData()
  } else {
    alert(res.message || 'Failed to delete subtype.')
  }
}
...
.category-tag { background: #f1f5f9; color: #0f172a; padding: 4px 8px; border-radius: 6px; font-size: 12px; font-weight: 700; }
