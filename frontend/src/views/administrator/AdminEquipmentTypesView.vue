<template>
  <MainLayout title="Equipment Types">
    <div class="admin-reference-container">
      
      <!-- Header Action Bar -->
      <div class="header-action-bar">
        <div>
          <h2>Equipment Types Management</h2>
          <p class="subtitle">Maintain high-level equipment categories (e.g. ICT, Communications).</p>
        </div>
      ...
      <div class="filter-controls-card">
        <div class="filter-group search-group">
          <label for="eqTypeSearch">Search</label>
        ...
        <div class="filter-group">
          <label for="eqTypeStatus">Status</label>
          <select id="eqTypeStatus" v-model="filterStatus" @change="currentPage = 1" class="input-select">
      ...
      <div class="table-card">
        ...
        <div v-else-if="sortedList.length === 0" class="empty-state">
          <p v-if="searchQuery || filterStatus !== 'all'">No equipment types match your filters.</p>
          <p v-else>No equipment types registered yet.</p>
        </div>
              ...
              <tr v-for="item in paginatedList" :key="item.id">
                ...
                <td class="text-center">
                  <div class="action-buttons">
                    <button class="icon-btn edit-btn" title="Edit Equipment Type" @click="openEditModal(item)">
                      <ion-icon :icon="createOutline" />
                    </button>
                    <button class="icon-btn delete-btn" title="Delete Equipment Type" @click="handleDelete(item)">
      ...
      <div v-if="showModal" class="modal-backdrop">
        <div class="modal-card">
          <div class="modal-header">
            <h3>{{ isEdit ? 'Edit Equipment Type' : 'Add New Equipment Type' }}</h3>
            <button type="button" class="close-btn" @click="showModal = false">&times;</button>
          </div>

          <form @submit.prevent="handleSave" class="modal-body">
            <div class="form-group">
              <label for="typeName">Type Name <span class="required-star">*</span></label>
              <input id="typeName" v-model="form.name" type="text" placeholder="e.g. ICT, Communications" required class="input-text" />
            </div>

            <div class="form-group">
              <label for="typeCode">Type Code</label>
              <input id="typeCode" v-model="form.code" type="text" placeholder="e.g. ICT, COMM" class="input-text code-font" />
            </div>
...
function openEditModal(item: EquipmentType) {
  isEdit.value = true
  form.value = {
    id: item.id,
    name: item.name,
...
async function handleSave() {
  if (!form.value.name.trim()) {
    modalError.value = 'Equipment type name is required.'
    return
  }
  ...
  const res = await saveEquipmentType({
  ...
  if (res.success) {
    ...
    modalError.value = res.message || 'Failed to save equipment type.'
  }
}

async function handleDelete(item: EquipmentType) {
  if (!confirm(`Are you sure you want to delete equipment type '${item.name}'?`)) return

  const res = await deleteEquipmentType(item.id)
  if (res.success) {
    loadData()
  } else {
    alert(res.message || 'Failed to delete equipment type.')
  }
}
