<template>
  <MainLayout title="Outgoing Communications" username="Admin">
    <div class="view-container">
      <!-- Header -->
      <div class="module-header-bar">
        <div>
          <h2>Outgoing Communications</h2>
          <p class="subtitle">Record, dispatch, and log outgoing memorandums, letters, and released documents.</p>
        </div>
        <button class="add-btn" type="button" @click="openCreateModal">
          <ion-icon :icon="addOutline" />
          <span>Log Outgoing Communication</span>
        </button>
      </div>
      ...
      <div class="filter-bar">
        <div class="search-box">
          ...
          <input
            ...
            type="text"
            placeholder="Search outgoing subject, office, category..."
...
async function handleDelete(item: Communication) {
  if (confirm(`Are you sure you want to soft delete outgoing communication "${item.subject}"?`)) {
